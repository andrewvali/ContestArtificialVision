...
def _load_identities(idmetacsv):
    ...
    if vgg2gender is None:
        ...
        for line in arr:
            try:
                vggnum = int(line[0][1:])
                vgg2gender[vggnum] = (int(line[-1]),i)
                gender2vgg.append((int(line[-1]), vggnum))
                i += 1
            except ValueError:
                pass
...
class Vgg2DatasetGender:
    def __init__(self,
                ...
                csvmeta='/content/drive/MyDrive/Contest Artificial Vision/GenderRecognitionFramework/dataset/data/vggface2_data/annotations/train.detected.csv',
